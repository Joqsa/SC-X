...
-- SC X the most rushed and money making focused script available
...
if not game:IsLoaded() then
	...
	if game:IsLoaded() then
		if DebugMode then
			warn("SC X - AutoExec Detection false")
		end
		return
	else
		ExecutorAutomatic = true
		if DebugMode then
			warn("SC X - AutoExec Detection true")
		end
	end
end
if DebugMode then
	warn("SC X - Game Loaded")
end
local KeySys = game:GetObjects("rbxassetid://8065404628")[1]

if DebugMode then
	warn("SC X - Loaded KeyUI")
end
...
local Price = "$7.99"
local CustomFolderName = "SC X Custom Scripts"
...
if writefile and isfile and readfile and delfile then
	if isfile("SC X Keybind.txt") then
		if readfile("SC X Keybind.txt") then
			if readfile("SC X Keybind.txt") then
				Keybind = readfile("SC X Keybind.txt")
			else
				delfile("SC X Keybind.txt")
			end
		end
	else
		-- No file, default it 
...
local DetectionScripts = {
	...
	DBR = {
		...
		Description = "A SC X exclusive, by our developer, Urbanstorm",
		Games = {71315343},
		Premium = false,
		Loadstring = "https://raw.githubusercontent.com/Urbanstorms/SC X/main/DBR.lua",
	},
	Vega = {
		Name = "Vega Remover",
...
function AddPlayer(Player)
	...
	if Player.OsPlatform == "SC X" then
		NewPlr.SName.Text = Player.Name.." - Using SC X"
	end
	...
	if CheckWritefile() then
		if isfile(WallpaperFileName) then
			...
			if getsynasset then
				...
				writefile("SC X Wallpaper.txt",WallpaperFileName)
...
Domain.Home.WallpaperManage.None.Interact.MouseButton1Click:Connect(function()
	if CheckWritefile() then
		if isfile("SC X Wallpaper.txt") then
			delfile("SC X Wallpaper.txt")
...
function StartAntiKick()
	--hookfunction(LocalPlayer.Kick,(function() Notify("Anti Kick","SC X has avoided the game kicking you locally",4400697855) wait(9e9) end))
end
...
function LoadThemes()
	if DebugMode then
		warn("SC X - Loading Themes")
	end
	Toast("Loading "..Theme.Name.." theme to SC X")
...
function FirstCheck()
	if isfile("SC X Version.txt") or isfile("SC X Keybind.txt") then
		firsttime = false
	else
		firsttime = true
	end
...
function VersionCheck()
	if DebugMode then
		warn("SC X - Starting Version Check")
	end
	if isfile("SC X Version.txt") then
		if DebugMode then
			warn("SC X - Version File Existent")
		end
		if tostring(readfile("SC X Version.txt")) ~= tostring(Release) then
			Notify("Update installed","SC X was updated to "..ReleaseType.." "..Release,4400701828)
			local num = math.random(1,5)
			if num == 2 then
				PromptPremium()
			end
			writefile("SC X Version.txt",tostring(Release))
			if DebugMode then
				warn("SC X - New Version, writing over")
			end
			local HttpService = game:GetService("HttpService")
			local RequestEnabled = (syn and syn.request) or (http and http.request) or http_request
			if RequestEnabled and not UserIsPremium then
				if DebugMode then
					warn("SC X - Joining Discord")
				end
				RequestEnabled({
					Url = 'http://127.0.0.1:6463/rpc?v=1',
					Method = 'POST',
		...
		if DebugMode then
			warn("SC X - No Version File")
		end
		if not firsttime then
			if DebugMode then
				warn("SC X - Not First Time, notifying")
			end
			Notify("Update installed","SC X was updated to "..ReleaseType.." "..Release,4400701828)
			local num = math.random(1,5)
			if num == 2 then
				PromptPremium()
				if DebugMode then
					warn("SC X - Prompting Premium")
				end
			end
		end
		if DebugMode then
			warn("SC X - Could be first time, continuing")
		end
		local HttpService = game:GetService("HttpService")
		local RequestEnabled = (syn and syn.request) or (http and http.request) or http_request
		if RequestEnabled and not UserIsPremium then
		...
		if DebugMode then
			warn("SC X - Starting writefile")
		end
		writefile("SC X Version.txt",tostring(Release))
		if DebugMode then
			warn("SC X - Writing new version file")
		end
	end
end
...
function ContinueBoot()
	DMNReady = true
	if DebugMode then
		warn("SC X - Continuing Boot Process (begun)")
	end
	Domain.Premium.Robux.Text = "or 2899 Robux"
	FirstCheck()
	VersionCheck()
	if DebugMode then
		warn("SC X - Checked Version success")
	end
	...
	if getconnections then
		...
		if DebugMode then
			warn("SC X - Disabled ScriptContext Errors")
		end
	end
	...
	if DebugMode then
		warn("SC X - Increasing ZIndex")
	end
	if UserIsPro and ThemeEnabled then
		LoadThemes()
	end
	if DebugMode then
		warn("SC X - Loading Themes (if any)")
	end
	...
	StartPopUpFramework()
	if DebugMode then
		warn("SC X - Starting pop up framework")
	end
	StartAntiKick()
	StartAntiIdle()
	...
	if DebugMode then
		warn("SC X - Started AntiKick and AntiIdle")
	end
	...
	if not IgnorePlayerData then
		...
		if DebugMode then
			warn("SC X - Finished Player Data")
		end	
	end
	if not Connected then
		Toast("Unable to connect to SC X server","GothamBold")
		UserIsPremium = false
		UserIsPro = false
	end	
	if not ExecutorAutomatic or firsttime then
		if DebugMode then
			warn("SC X - First time or not autoexec")
		end
		local sound = Instance.new("Sound")
		sound.Parent = Domain
		...
		sound.Name = "boot"
		if DebugMode then
			warn("SC X - Instance Success!")
		end
		sound.Volume = 10
		sound.PlayOnRemove = true
		if DebugMode then
			warn("SC X - Waiting for sound load")
		end
		sound:Destroy()
		if DebugMode then
			warn("SC X - Destroyed sound (loaded)")
		end
		wait(0.75)
		if DebugMode then
			warn("SC X - Preparing effect")
		end
		...
		if DebugMode then
			warn("SC X - Keybind Note false")
		end
		if DebugMode then
			warn("SC X - Opening main")
		end
		OpenMain()
		if DebugMode then
			warn("SC X - Opened Main")
		end
	...
	Toast("Welcome to SC X v"..Release.. ", "..LocalPlayer.DisplayName,"GothamBold")
	...
	if CheckWritefile() then
		if isfile("SC X Wallpaper.txt") then
			if isfile(readfile("SC X Wallpaper.txt")) then
				if getsynasset then
					Domain.Home.Wallpaper.Visible = true
					Domain.Home.Wallpaper.Image = getsynasset(readfile("SC X Wallpaper.txt"))
					Domain.Home.WallpaperManage.WBG.WBox.Text = readfile("SC X Wallpaper.txt")
				else
					Domain.Home.Wallpaper.Visible = true
					Domain.Home.Wallpaper.Image = getcustomasset(readfile("SC X Wallpaper.txt"))
					Domain.Home.WallpaperManage.WBG.WBox.Text = readfile("SC X Wallpaper.txt")
				end
			end
		end
...
function CheckLatest()
	if DebugMode then
		warn("SC X - Checking version")
	end
	--loadstring(game:HttpGet('https://raw.githubusercontent.com/shlexware/SC X/main/routine',true))()
	--local ver = game:HttpGet('https://raw.githubusercontent.com/shlexware/SC X/main/latest')
	--print(ver)
	--if tonumber(ver) > Release then
	--	Connected = false
	--	warn("Not up to date, needs to be on "..tostring(game:HttpGet('https://raw.githubusercontent.com/shlexware/SC X/main/latest')))
	--	return false
	--else
	Connected = true
	--end
...
function BootSCX()
	if DebugMode then
		warn("SC X - Beginning Boot Process")
	end
	Domain.Main.Visible = true
	Domain.Main.Position = UDim2.new(0.5, 0, 1.05, 0)
	Domain.Main.KeybindNote.Position = UDim2.new(0.5,0,-1.303,0)
	Domain.Main.KeybindNote.Text = "Loading SC X"
	Domain.Main.KeybindNote.Visible = true
	...
	if DebugMode then
		warn("SC X - Continuing Boot Process")
	end
	ContinueBoot()
...
function ChangeKeybind(Key)
	...
	if CheckWritefile() then
		writefile("SC X Keybind.txt",tostring(NewKeyNoEnum))
...
game.Players.PlayerAdded:Connect(function(Player)
	...
	if Player.Name == "shlexius" then
		Toast("The developer of SC X, shlex, has joined your server")
	end
...
coroutine.wrap(function()
	wait(1000)
	Toast("Enjoying SC X? Let us know in our Discord!")
